...
const Table = (() => {
  ...
  if(typeof ESB_PLATFORM === 'string' && ESB_PLATFORM === 'browser') {
    platform = import('./platform/browser/index.js');
  } else {
    platform = import('./platform/node/index.js');
  }  
  ...
  return class {
    ...
    constructor(name, {root = '.db', meta = '.meta', database, indexes} = {}) {
      ...
      this.#ready = platform.then(({createTable}) => {
        return createTable(this, root, meta);
      }).then((res) => {
        this._storage = res;
      });
    }
    ...
    async getRecords({filter, sorter, skip, limit, filterIndexes, rawSorter} = {}) {
      await this.#ready;
      const {getRecords} = await platform;
    ...
    async save(records = [], countResult = false) {
      ...
      const {flushData} = await platform;
